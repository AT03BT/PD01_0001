/*
    wwwroot/js/interactiveelement/taskmanager.js
    Version: 0.3.14 // Version increment for robust ID handling and ensuring child added to plane
    ...
    ============
*/
import { TaskQueue } from './taskqueue.js'; // Corrected path
...
export const TaskManager = {
    ...
    dispatchInputEvent: function (event) {
        try {
            if (event.type !== 'mousemove') {
                ...
                // NEW: Use event.target.tagName for more reliable target element logging
                console.log('Target element:', event.target.tagName || event.target.nodeName || event.target);
    ...
    yieldCurrentTask: function (taskThatYielded) { // NEW: Accept taskThatYielded parameter
        if (this.currentTask) {
            console.log(`TaskManager: Current task (${this.currentTask.constructor.name}) yielded control.`);

            // Add the _implement of the yielded GeometricConstruction to the GeometricPlane.
            // Use the ID generated by the Construction itself and its _implement.
            if (taskThatYielded && taskThatYielded._implement && taskThatYielded._implement.id) { // Use taskThatYielded
                this.rootGeometricPlane.addChild(taskThatYielded._implement.id, taskThatYielded._implement);
            } else {
                console.warn(`TaskManager: Yielded task (${this.currentTask.constructor.name}) _implement is missing an ID or is not meant to be added directly to GeometricPlane.`);
            }

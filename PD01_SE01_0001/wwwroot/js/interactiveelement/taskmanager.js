    ...
    yieldCurrentTask: function () {
        if (this.currentTask) {
            ...
            if (this.currentTask._implement && this.currentTask._implement.id) {
                ...
                // If a task yields but its implement is missing ID (e.g., RectangleConstruction's corners),
                // it might not be meant to be added to the plane directly. Log for diagnostic.
                console.warn(`TaskManager: Yielded task (${this.currentTask.constructor.name}) _implement is missing an ID or is not meant to be added directly to GeometricPlane. Check its lifecycle.`);
            }
